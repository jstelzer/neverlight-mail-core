...
use super::types::{OAuthError, OAuthMetadata};

/// Discover OAuth authorization server metadata from a JMAP session URL.
///
/// Discovery chain:
/// 1. GET `jmap_session_url` without auth → expect 401 with `WWW-Authenticate`
...
pub async fn discover_oauth_metadata(jmap_session_url: &str) -> Result<OAuthMetadata, OAuthError> {
    ...
    let resource_metadata_url = discover_resource_metadata_url(&http, jmap_session_url).await?;
...
/// Step 1: GET the session URL without auth, parse `WWW-Authenticate` for resource metadata.
async fn discover_resource_metadata_url(
    ...
    if resp.status() != reqwest::StatusCode::UNAUTHORIZED {
        return Err(OAuthError::Discovery(format!(
            "Expected 401 from unauthenticated session request, got {}",
            resp.status()
        )));
    }
