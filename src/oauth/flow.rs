...
use super::exchange::exchange_code;
use super::pkce::{generate_code_verifier, pkce_challenge_s256};
use super::redirect::OAuthRedirectHandler;
use super::registration::register_client;
use super::types::{AppInfo, OAuthError, OAuthMetadata, TokenSet};
...
impl OAuthFlow {
    ...
    ///
    /// `jmap_session_url` is the JMAP session endpoint (e.g. `https://api.fastmail.com/jmap/session`).
    pub async fn discover_and_register(
        jmap_session_url: &str,
        redirect_uri: &str,
    ) -> Result<Self, OAuthError> {
        log::info!("Starting OAuth discovery for {jmap_session_url}");
        let metadata = discover_oauth_metadata(jmap_session_url).await?;
        ...
        let registration = register_client(&metadata, &app_info).await?;
        ...
        Ok(Self {
            ...
            resource: jmap_session_url.to_string(),
            redirect_uri: redirect_uri.to_string(),
            pkce_verifier,
            state,
            metadata,
    ...
    pub fn authorization_url(&self) -> String {
        ...
        format!(
            ...
            urlencod(&self.state),
            urlencod(&challenge),
            urlencod("urn:ietf:params:oauth:scope:mail"),
